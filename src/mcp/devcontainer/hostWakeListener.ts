...
async function handleWake(msg: WakeMessage): Promise<void> {
	...
	try {
		...
		if (!sessionExists) {
			const claudeCommand = `claude --model default --effort max --dangerously-skip-permissions --dangerously-load-development-channels plugin:agent-team-bridge@agent-team-bridge`;
			await execInContainer({
				projectPath: resolved,
				command: [
					"tmux",
