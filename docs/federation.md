...
- The Gateway's `cross_domain_share` value op writes its own mirror and then the Router record in one operation. The record is what admits the friend's rows, so it never outlives the mirror: a mirror the Gateway's migration fence refuses answers `migrating` before any Router traffic, and a refused record removes the mirror it was written for and answers the value op as failed. The phone posts nothing itself. Unshare withdraws the record first, so a fenced mirror leaves only a share the Router no longer admits until the retry.
...
- A gateway's presented admission stays on its connection record and is re-resolved against the current revocations on every frame; a revoked signer's connections leave through the one drop path. The bridge's refusals (`not_registered`, `gateway_not_admitted`, `inbox_unavailable`, `stale_incarnation`) are declared in `wire-vocabulary.ts`.
