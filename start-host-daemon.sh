...
tmux new-session -d -s "$TMUX_SESSION" "bash -c 'source ~/.bashrc; claude --name ${HOST_NAME} --model default --effort low --dangerously-skip-permissions --dangerously-load-development-channels plugin:agent-team-bridge@agent-team-bridge; exec bash'"
